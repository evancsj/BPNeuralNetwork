...
for i = 1:m
    ...
    a2 = [1;a2];
    for k = 1:K
        J = J+1/m*(0-yi(k)*log(sigmoid(Theta2(k,:)*a2)) ...
            -(1-yi(k))*log(1-sigmoid(Theta2(k,:)*a2)));
    end
end

for j = 1:hidden_layer_size
    for k = 2:input_layer_size+1
        J = J+lambda/(2*m)*Theta1(j,k)*Theta1(j,k);
    end
end

for j = 1:num_labels
    for k = 2:hidden_layer_size+1
        J = J+lambda/(2*m)*Theta2(j,k)*Theta2(j,k);
    end
end
